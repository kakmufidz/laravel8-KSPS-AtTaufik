...
@section('konten')
    ...
    <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Angsuran Mudorobah</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
          ...
          <div class="col-md-12">
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">History Angsuran Mudorobah</h3>
              </div>
              <!-- /.card-header -->
              @if (session('status'))
              ...
              <div class="card-body">
                <table class="table table-striped table-bordered">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$no++}}</td>
                      <td>A300{{$datas->id_angsuran_mudorobah}}</td>
                      <td>{{$datas->no_anggota}} - {{$datas->name}}</td>
                      <td>{{$datas->tgl}}</td>
                      <td>{{$datas->data_mudorobah}}</td>
                      <td>@currency($datas->jumlah)</td>
                      <td>{{$datas->keterangan}}</td>
                      {{-- <td>
                        <div class="text-right">
                          <a href="/angsuran/mudorobah/edit/{{$datas->id_angsuran_mudorobah}}" class="btn btn-sm btn-success">
                            <i class="fas fa-pencil-alt"></i>
                          </a>
                          <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#modal-hapus" onclick="pass_id_to_modal({{$datas->id_angsuran_mudorobah}})">
...
@section('script')
    <script type="text/javascript">
      function pass_id_to_modal(id) {
        ...
        delete_button.href = "/angsuran/mudorobah/hapus/" + id;
      }
    </script>
@endsection