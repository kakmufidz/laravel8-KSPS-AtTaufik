...
<div class="wrapper">
  <div class="container">
    <!-- Main content -->
    @foreach ($keuntungan as $keuntungans)
        ...
        <div class="col-12" style="padding-left:30mm;padding-right:30mm;">
          ...
          <div class="col-12">
            <div class="text-center ">
              <h2>BUKTI SETORAN KEUNTUNGAN MUDOROBAH
                <h5>NO: U100{{$keuntungans->id_keuntungan}}</h5>
              </h2>
            </div>
            <div class="col-12">
              <table class="table">
                <tr>
                  ...
                  <td class="w-75">: {{$keuntungans->id_anggota}} - {{$keuntungans->name}}</td>
                </tr>
                <tr>
                  <th class="w-25">Tanggal Transaksi</td>
                  <td class="w-75">: {{$keuntungans->tgl}}</td>
                </tr>
              </table>
            </div>
            <div class="col-12">
              <table class="table table-bordered">
                ...
                @php $no=1; @endphp
                @forelse ($keuntungan as $keuntungans)
                <tbody>
                  <tr>
                    <td>{{$no++}}</td>
                    <td>{{$keuntungans->data_mudorobah}}</td>
                    <td>@currency($keuntungans->jumlah)</td>
                    <td>{{$keuntungans->keterangan}}</td>
                  </tr>
                </tbody>
                @empty
            ...
            <div class="row">
              <div class="col-12" style="padding-left: 20px">
                <h5>Total keuntungan = <b>Rp @currency($keuntungans->jumlah)</b></h5><br><br>
              </div>
            </div>
          </div>
        </div>
